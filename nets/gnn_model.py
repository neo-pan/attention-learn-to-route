...
from nets.gnn_encoder import GraphTransformerEncoder, GraphTransformerFFEncoder
...
class GNNModel(nn.Module):
    def __init__(
        ...
        self.embedder = GraphTransformerEncoder(
            n_heads=n_heads,
    ...
    def forward(self, input, return_pi=False):
        """
        :param input: input node features in the form of `torch_geometric.data.Batch`
        :param return_pi: whether to return the output sequences, this is optional as it is not compatible with
        using DataParallel as the results may be of different lengths on different GPUs
        :return:
        """
        assert isinstance(input, Batch)
...
class GNNFFModel(GNNModel):
    def __init__(
        ...
        self.embedder = GraphTransformerFFEncoder(
            n_heads=n_heads,
            embed_dim=embedding_dim,
            n_layers=self.n_encode_layers,
            normalization=normalization,
